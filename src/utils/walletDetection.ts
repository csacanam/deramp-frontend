...
export function getWalletDeepLink(walletId: string, url: string): string | null {
  ...
  switch (walletId) {
    case 'metaMask':
      // MetaMask mobile: try different deep link formats
      // First try the Daimo Pay format, then fallback to direct
      const metamaskUrl = `metamask://dapp?url=${encodedUrl}`;
      console.log(`🦊 MetaMask deep link:`, metamaskUrl);
      return metamaskUrl;
    case 'coinbaseWallet':
